...
From semantics.pl.logrel Require Import notation.
...
(** In our operational semantics, only certain terms are atomically comparable.
  This also restricts the types on which we can CAS.
 *)
Inductive ComparableLit : type → Prop :=
  | comp_unit : ComparableLit TUnit
  | comp_int : ComparableLit Int
  | comp_bool : ComparableLit Bool
  | comp_ref A : ComparableLit (Ref A).
Inductive Comparable : type → Prop :=
  | comp_sum A B :
      ComparableLit A →
      ComparableLit B →
      Comparable (A + B)
  | comp_lit A : ComparableLit A → Comparable A.
...
Inductive syn_typed : nat → (gmap string type) → expr → type → Prop :=
  ...
  | typed_fork n Γ e A :
      TY n; Γ ⊢ e : A →
      TY n; Γ ⊢ Fork e : Unit
  | typed_cmpxchg n Γ e1 e2 e3 A :
      Comparable A →
      TY n; Γ ⊢ e1 : Ref A →
      TY n; Γ ⊢ e2 : A →
      TY n; Γ ⊢ e3 : A →
      TY n; Γ ⊢ CmpXchg e1 e2 e3 : (A × Bool)
